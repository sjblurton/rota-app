# Shared Routes Folder Agent Guide

## Purpose

This folder contains shared route modules and middleware for the backend. It is responsible for:

- Composing and exporting Express routers for use in the main app entrypoint.
- Mounting feature routers from `src/api/{module}/routes`.
- Defining and colocating middleware that is specific to a route or role (e.g. admin, superadmin).

## Guidelines

- Each role or shared route group (e.g. admin, superadmin) should have its own router file (e.g. `admin-router.ts`).
- Middleware that is only used by a specific route or role should be placed in a `middleware/` subfolder next to the router.
- Routers in this folder should:
  - Import feature routers from `src/api/{module}/routes` and mount them at the correct path.
...
- Do not place business logic, validation, or database access in this folder—only routing and middleware composition.
- Do not import services or repositories directly—only controllers and routers from the API modules.
- Keep route and middleware boundaries clear and colocate related middleware with its router.
...
- See src/api/{module}/routes for feature routers.
