...
describe('swapRequestBodySchema', () => {
  it('accepts valid target_staff_id', () => {
    expect(swapRequestBodySchema.safeParse({ target_staff_id: 'staff-1' }).success).toBe(true);
  });
...
describe('createStaffBodySchema', () => {
  it('accepts valid payload', () => {
    expect(createStaffBodySchema.safeParse({ name: 'Alice', phone_number: '+441234567890' }).success).toBe(true);
  });
  it('rejects empty name', () => {
    expect(createStaffBodySchema.safeParse({ name: '', phone_number: '+441234567890' }).success).toBe(false);
  });
...
describe('createShiftBodySchema', () => {
  it('accepts valid shift payload', () => {
    expect(createShiftBodySchema.safeParse({ staff_id: 'staff-1', start_time: '2024-01-01T08:00:00Z', end_time: '2024-01-01T16:00:00Z' }).success).toBe(true);
  });
  it('rejects start_time after end_time', () => {
    expect(createShiftBodySchema.safeParse({ staff_id: 'staff-1', start_time: '2024-01-01T16:00:00Z', end_time: '2024-01-01T08:00:00Z' }).success).toBe(false);
  });
  it('rejects non-UTC datetime', () => {
    expect(createShiftBodySchema.safeParse({ staff_id: 'staff-1', start_time: '2024-01-01T08:00:00+01:00', end_time: '2024-01-01T16:00:00Z' }).success).toBe(false);
  });
});

describe('updateShiftBodySchema', () => {
  it('accepts partial update with start_time only', () => {
    expect(updateShiftBodySchema.safeParse({ start_time: '2024-01-01T08:00:00Z' }).success).toBe(true);
  });
  it('accepts valid time range update', () => {
    expect(updateShiftBodySchema.safeParse({ start_time: '2024-01-01T08:00:00Z', end_time: '2024-01-01T16:00:00Z' }).success).toBe(true);
  });
  it('rejects empty payload', () => {
    expect(updateShiftBodySchema.safeParse({}).success).toBe(false);
  });
  it('rejects start_time after end_time', () => {
    expect(updateShiftBodySchema.safeParse({ start_time: '2024-01-01T16:00:00Z', end_time: '2024-01-01T08:00:00Z' }).success).toBe(false);
  });
});
