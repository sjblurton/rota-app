...
export const timeRangeFilterSchema = z.object({
  start_time: utcDateTimeSchema.optional().describe(timeDescription("start_time")),
...
const commonShiftFiltersSchema = z.object({
  ...
  staff_id: z.string().optional().describe("Filter by the assigned staff member ID"),
});
...
export const swapsListQuerySchema = z.object({
  ...
  ...timeRangeFilterSchema.shape,
  ...commonShiftFiltersSchema.shape,
  status: swapRequestStatusEnum.optional().describe("Filter by swap request status"),
});
